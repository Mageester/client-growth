import {
  flagCommercialLanguage,
  type CommercialLanguageMatch,
} from "@/core/commercialLanguage";
...
/**
 * Flag offering lines that look like marketing copy rather than work customers
 * buy.
 *
 ...
 *
 * The phrase taxonomy itself lives in core (commercialLanguage.ts) because the
 * offering-suggestion path has to apply exactly the same judgment to a site's
 * own navigation. Advisory only: it returns what to say, changes nothing, and
 * blocks no save.
 */
export type OfferingWarning = CommercialLanguageMatch;

export function offeringWarnings(offerings: string[]): OfferingWarning[] {
  return flagCommercialLanguage(offerings);
}
