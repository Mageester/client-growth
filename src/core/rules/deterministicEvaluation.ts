 ...
 * draft.
 *
 * So the rule carries its own judgment, deterministically. Returning null means
 * "this candidate does need a model", which is the case for every other rule.
 *
...
export function deterministicEvaluationFor(candidate: Candidate): Evaluation | null {
  if (candidate.ruleId !== "no-service-pages") return null;
